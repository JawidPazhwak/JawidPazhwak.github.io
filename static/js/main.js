...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadAudioFiles = async () => {
        try {
            ...
            console.log('Fetching audio_files.json...');
            ...
            if (!Array.isArray(data.files)) {
                throw new Error('Invalid JSON structure: files is not an array');
            }
            
            if (data.files.length === 0) {
                throw new Error('No audio files found in configuration');
            }
