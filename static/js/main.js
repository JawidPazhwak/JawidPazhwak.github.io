...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadAudioFiles = async () => {
        try {
            ...
            const response = await fetch('static/uploads/audio/tracks.json');
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            const data = await response.json();
            ...
            const audioFiles = data.tracks;
            console.log(`Found ${audioFiles.length} tracks to load`);
            ...
            for (const [index, file] of audioFiles.entries()) {
                console.log(`Processing file ${index + 1}/${audioFiles.length}:`, file);
                
                const trackId = `track-${index + 1}`;
                const filePath = `static/uploads/audio/${file.file}`;
                console.log('File path:', filePath);
                
                // Verify file exists before creating element
                try {
                ...
                trackElement.innerHTML = `
                    <div class="music-track">
                        ...
                        <p class="neon-text">${file.description}</p>
                ...
                if (audioElement) {
                    audioElement.addEventListener('error', (e) => {
                        ...
                        playerDiv.innerHTML = `
                            <div class="alert alert-danger">
                                ...
                                <br>
                                <small>Error details: ${e.message}</small>
                            </div>
                        `;
                    });
                    ...
                    audioElement.addEventListener('loadstart', () => {
                        console.log(`Started loading audio file: ${filePath}`);
                    });

                    audioElement.addEventListener('stalled', () => {
                        console.warn(`Audio loading stalled: ${filePath}`);
                    });
